...
download_file <- function(folder, type) {
  ...
  while (is.na(name)) {
    ...
    if (name != "") {
      ...
      if (count(file) > 1) {
        print("ERROR: Please specify which folder the file is in. For example, Proccessed Data/new_data.xlsx")
        name <- NA
      ...
      path <- drive_find(pattern = folder, shared_drive = "SWEL Lab", type = "folder")
  ...
  drive_download(as_id(mv_id), path = temp_file, overwrite = TRUE)
  return (temp_file)
}

#this func is very dependent on the method val file being the exact format of how
# they are generated by method_val_processing.R
import_method_val <- function() {
  ...
  mv_file <- download_file("Method val tests", "Method val")
  
  #read slopes into dataframe
  sheets <- getSheetNames(mv_file)
  name <- "Slope and R"
  ...
  return (list(expected_native, expected_ISTD, slope_df, limits_df, mv_name))
}
...
main <- function() {
  ...
  data_file <- download_file("Data processing tests", "raw data")
  ...
  expected_native <- as.data.frame(mv_data[1])[, mapping[["Analyte"]]]
  expected_ISTD <- as.data.frame(mv_data[2])[, mapping[["ISTD"]]]
  slope_df <- as.data.frame(mv_data[3])[, mapping[["Analyte"]]]
  limits_df <- as.data.frame(mv_data[4])[, mapping[["Analyte"]]]
  mv_name <- mv_data[[5]]
  LOD <- limits_df["LOD", ]
  LOB <- limits_df["LOB", ]
  LOQ <- limits_df["LOQ", ]
  ...
  LOD_style <- createStyle(fgFill = "#afffaf")
  for (i in seq_len(nrow(LOQ_flags))) {
    for (j in seq_len(ncol(LOQ_flags))) {
      if (LOQ_flags[i, j] == "<LoQ") {
        ...
        #print("added style")
      } else if (export_df[i,j] == "<LoD") {
        addStyle(wb, "Limit of Quantification", style = LOD_style, rows = i + 1, cols = j + 1, gridExpand = FALSE, stack = TRUE)
      }
    }
  }
