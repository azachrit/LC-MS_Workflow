...
import_method_val <- function() {
  ...
  mv_name <- NA
  while (is.na(mv_name)) {
    mv_name <- readline(prompt = "Enter the method val name or press enter to use most recent method val: ")
    
    ### NEED TO ADD ERROR CHECKING HERE TO MAKE SURE NAME IS VALID/FILE EXISTS
    if (mv_name == "") {
      mv_id <- unlist(files[[1, "id"]])
      mv_name <- unlist(files[1, "name"])
    }
    else {
      mv_id <- unlist(files[files[,1] == mv_name, "id"])
    }
  }
