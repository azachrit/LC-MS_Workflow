...
import_method_val <- function() {
  file_md <- download_file("Tests", "Method val tests", "method validation")
  temp_file <- file_md[[1]]
  mv_name <- file_md[[3]]
...
format_df <- function(df) {
  df <- as.data.frame(df) %>% mutate(across(everything(), as.numeric)) %>%
    rename_with(~ str_replace(., "^X", ""), starts_with("X"))
  df <- df[, mapping[["Analyte"]]]
  return (df)
}
...
main <- function() {
  ...
  file_md <- download_file("Tests","Data processing tests", "raw data")
  temp_file <- file_md[[1]]
  file_id <- file_md[[2]]
  ...
  get_shared_vars(all_data, FALSE)

  #get expected values from relevant (most recent) method val
  mv_data <- import_method_val()
  expected_native <- format_df(mv_data[1])
  expected_ISTD <- format_df(mv_data[2])
  slope_df <- format_df(mv_data[3])
  limits_df <- format_df(mv_data[4])
  mv_name <- mv_data[[5]]
  LOD <- limits_df["LOD", ]
  LOB <- limits_df["LOB", ]
  LOQ <- limits_df["LOQ", ]
  ...
  rownames(blank_sub_df_1) <- rownames(no_1ngml_data)
  rownames(blank_sub_df_2) <- rownames(no_1ngml_data)
